                        ...
                        <td class="p-4">
                            @if($order->status == 'dibayar')
                                @if($order->status_pengiriman == 'dikirim')
                                    <span class="text-green-600 font-bold text-xs">Dikirim @if($order->nomor_resi) (Resi: {{ $order->nomor_resi }}) @endif</span>
                                @else
                                    <form action="{{ route('admin.pesanan.kirim', $order->id) }}" method="POST" class="flex flex-col gap-2 min-w-[200px]">
                                        ...
                                        <div class="flex gap-1">
                                            <input type="text" name="nomor_resi" placeholder="No Resi/Plat (Opsional)" class="border border-gray-300 rounded px-2 py-1 text-xs flex-1 w-full">
